...
import { Paperclip, X, Plus, Mic, ArrowRight, ExternalLink, Video, Crown, Music, FileText } from "lucide-react";
...
export default function ChatInput({
  ...
  const handleSend = () => {
    if ((value.trim() || attachedFile || selectedMedia.length > 0) && !disabled) {
      onSend();
      setAttachedFile(null);
      setSelectedMedia([]);
    }
    ...
    <div className="w-full">
      <div className="relative">
        ...
        <div className="relative rounded-2xl bg-gradient-to-r from-[#d1f5fa] to-purple-200 p-[1.5px]">
          <div className="bg-white backdrop-blur-lg rounded-[15px] p-4">
            <div className="relative">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button 
                    onClick={handleSend}
                    disabled={disabled || (!value.trim() && !attachedFile && selectedMedia.length === 0)}
                    className={`rounded-full p-2 transition-colors ${
                      (value.trim() || attachedFile || selectedMedia.length > 0) && !disabled
                ...
                <div className="flex flex-wrap gap-2 mt-3 bg-gradient-to-r from-[#defbff] to-purple-100 -mx-4 -mb-4 px-4 pb-4 pt-3 rounded-b-2xl">
                  {selectedMedia.map((media) => (
                    <div key={media.id} className="relative group">
                                              <div 
                          className="w-16 h-16 bg-white rounded-lg border-2 border-gray-200 overflow-hidden shadow-sm hover:shadow-lg hover:scale-105 transition-all duration-200 cursor-pointer"
